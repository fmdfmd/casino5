					...
					<section className={styles.dashboard}>
						...
						<div className={styles.campaignTable}>
							...
							{campaigns.map((camp) => (
								<div
									key={camp.id}
									className={`${styles.glassCard} ${expandedCard === camp.id ? styles.active : ''}`}
								>
									<div
										className={styles.cardHeader}
										onClick={() =>
											setExpandedCard(expandedCard === camp.id ? null : camp.id)
										}
									>
										...
										<div className={styles.commCol}>
											...
											<span
												className={`${styles.arrow} ${expandedCard === camp.id ? styles.up : ''}`}
											>
												<svg
													width='12'
													height='8'
										...
										<div className={styles.detailsContent}>
											...
											<div className={styles.linkSection}>
												...
												<div className={styles.linkRow}>
													<input
														readOnly
														value={`site.com/?c=${camp.name.toLowerCase()}`}
													/>
													<button
														className={styles.copyBtn}
														onClick={() =>
															copyToClipboard(
																`site.com/?c=${camp.name.toLowerCase()}`,
															)
														}
													>
														Copy Link
					...
					<div className={styles.modalBox} onClick={(e) => e.stopPropagation()}>
						...
						<div className={styles.modalBody}>
							<p className={styles.questionText}>
								Do you want to create a new tracking link?
							</p>
							...
							<div className={styles.modalActions}>
								...
								<button className={styles.getOutBtn}>Create</button>
							</div>
						</div>
					</div>
				</div>
