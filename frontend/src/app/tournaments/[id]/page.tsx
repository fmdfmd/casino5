...
export default function TournamentIdPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!currentTournament) {
        return (
            <div className={styles.notFoundWrapper} style={{ color: 'white', textAlign: 'center', paddingTop: '100px' }}>
                <h1>Tournament not found</h1>
                <p>ID in URL: {resolvedParams?.id}</p>
                <Link href="/tournaments" style={{ color: '#00ff00', textDecoration: 'underline' }}>
            ...
            <div className={styles.wrapper}>
                ...
                <main className={styles.mainContent}>
                    ...
                    <div className={styles.topNav}>
                         <Link href="/tournaments" className={styles.backLink}>TOURNAMENTS</Link>
                    </div>
                    ...
                    <div className={styles.titleSection}>
                        <h1 className={styles.tournamentTitle}>{currentTournament.title}</h1>
                        <div className={styles.prizeLabel}>Prize pool</div>
                        <div className={styles.prizeValue}>{currentTournament.prizePool}</div>
                    </div>

                    <div className={styles.rulesCard}>
                        <div className={styles.rulesBlock}>
                            <div className={styles.ruleRow}><strong>Dates:</strong><p>{currentTournament.dates}</p></div>
                            <div className={styles.ruleRow}><strong>How to place a bet:</strong><p>Minimum Bet: {currentTournament.minBet}</p></div>
                            <div className={styles.ruleRow}><strong>How to get points:</strong><p>{currentTournament.scoringRules}</p></div>
                            <p className={styles.smallNote}>Time to award players 72 hours after the tournament termination.</p>
                        </div>
                        <button className={styles.loginButton}>Log in/Sign up</button>
                    </div>

                    {/* === TERMS & CONDITIONS === */}
                    <div className={`${styles.accordionCard} ${!isTermsOpen ? styles.closed : ''}`}>
                        <div 
                        ...
                        <div className={`${styles.collapsibleContent} ${isTermsOpen ? styles.contentOpen : ''}`}>
                    ...
                    {/* === LEADERBOARDS === */}
                    <div className={`${styles.accordionCard} ${!isLeaderboardOpen ? styles.closed : ''}`}>
                        <div 
                        ...
                        <div className={`${styles.collapsibleContent} ${isLeaderboardOpen ? styles.contentOpen : ''}`}>
                            <div className={styles.leaderboardWrapper}>
                                <div className={styles.tableHeader}>
                                    <span>Place</span><span>Name</span><span>Prize</span>
                                </div>
                                <div className={styles.leaderboardList}>
                                    {currentTournament.leaderboard.length > 0 ? (
                                        currentTournament.leaderboard.map((user) => (
                                            <div key={user.place} className={`${styles.row} ${user.isTop ? styles.topRow : ''}`}>
                                                <div className={styles.placeCell}>{renderPlace(user.place)}</div>
